...
function openConcept(index) {
  try {
    ...
    if (typeof setMobileNavActive === "function") setMobileNavActive("learn");
...
function openPracticeBook() {
  try {
    ...
    if (typeof setMobileNavActive === "function") setMobileNavActive("practice");
  } catch (err) {
...
function normalizePracticeBookOptions() {
  ...
  units.forEach((u) => {
    ...
    u.mcqs.forEach((q) => {
      ...
      if (q.correct && String(q.correct).includes("<") && !String(q.correct).includes("&lt;")) {
  ...
  console.log("✅ MCQ options HTML-escaped (fixes Q40/Q41/Q42 empty options)");
}
...
console.log("✅ app-fixes.js v7 – nav fix phone+laptop + Jupyter cells");
